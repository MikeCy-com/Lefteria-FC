      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-base font-bold text-white leading-tight">{user?.name || "Coach"}</h1>
          </div>
        </div>
          ...
          {allGroups.map(g => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 text-left min-w-0">
                  <p className="text-sm font-semibold text-white group-hover:text-blue-400 transition-colors truncate">{g.name}</p>
            ...
            {trainingSessions.slice(0, 3).map(session => (
              <div key={session.id} className="bg-[#111] border border-white/[0.06] rounded-2xl p-3 flex items-center gap-3" data-testid={`coach-training-${session.id}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-zinc-500 mt-0.5">
                    {session.date && new Date(session.date + "T00:00:00").toLocaleDateString("el-GR", { weekday: "short", day: "numeric", month: "short" })}
